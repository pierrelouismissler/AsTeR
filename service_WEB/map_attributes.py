...
class Map(object):
    ...
    @property
    def js(self):
        return Markup(self.render('maps/gmapjs.html', gmap=self))

    @property
    def html(self):
        return Markup(self.render('maps/gmap.html', gmap=self))
...
class GoogleMaps(object):
    ...
    def register_blueprint(self, app):
        module = Blueprint("maps", __name__,
...
# class Map(object):
#     def __init__(self, identifier, lat, lng,
#                  zoom=13, maptype="TERRAIN", markers=None,
#                  varname='map',
...
#         self.markers = markers or []
#         if isinstance(markers, list):
#             self.markers = {'http://maps.google.com/mapfiles/ms/icons/red-dot.png': markers}
#         self.identifier = identifier
#         if 'infobox' in kwargs:
#             self.infobox = kwargs['infobox']
#             # jinja2 has no builtin for type so a flag is set to check if infobox is
