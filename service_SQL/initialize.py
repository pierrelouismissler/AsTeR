...
class SimulationCalls:
    ...
    def __init__(self, n_steps, map_center=None, map_radius=None,  \
...
class BayAreaFilter:
    ...
    def random_attributes():
        ...
        return a + '-' + b + '-' + c, np.random.uniform(0, 3)
...
if __name__ == '__main__':
    ...
    dtf = flt.run(dtf)
    att = np.vstack([flt.random_attributes() for _ in range(len(dtf))])
    dtf['phone_number'] = att[:,0]
    dtf['priority'] = att[:,1]
    # Serialize general initialization configuration
    with open('configs/basic_config.yaml') as raw: tst = yaml.safe_load(raw)
    res = dtf.to_dict(orient='index').values()
    for dic in res: dic.update({'call_id': uuid.uuid4().hex})
    tst['calls'] = list(res)
    arg = {'ensure_ascii': False, 'indent': 4}
    with open('configs/simulation.json', 'w', encoding='utf-8') as f: json.dump(tst, f, **arg)